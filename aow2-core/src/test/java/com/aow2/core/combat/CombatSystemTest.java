...
class CombatSystemTest {
    ...
    void shouldPerformDirectAttack() {
        var attacker = new Unit(1, UnitType.CONFED_ZEUS, zeusStats, 0, new GridPosition(10, 10));
        var target = new Unit(2, UnitType.CONFED_INFANTRY, infantryStats, 1, new GridPosition(11, 10));
        combatSystem.performAttack(attacker, target);
        assertEquals(37, target.hp());
    ...
    void shouldAddExperienceOnHit() {
        ...
        var target = new Unit(2, UnitType.CONFED_INFANTRY, infantryStats, 1, new GridPosition(11, 10));
        combatSystem.performAttack(attacker, target);
        assertEquals(1, attacker.experience());
    }

    @Test
    @DisplayName("Infantry deals reduced damage to buildings (50% multiplier)")
    void shouldDealReducedDamageToBuildingsFromInfantry() {
        var attacker = new Unit(1, UnitType.CONFED_INFANTRY, infantryStats, 0, new GridPosition(10, 10));
